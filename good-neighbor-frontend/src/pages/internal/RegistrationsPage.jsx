  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-neutral-900">OSBB Registration Requests</h2>
          ...
          <table className="min-w-full divide-y divide-neutral-200">
            <thead className="bg-neutral-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-neutral-700 uppercase">Head</th>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Registration Details #{selectedRegistration.id}</h3>
              <button
            ...
            <div className="space-y-4">
              <div><strong>EDRPOU:</strong> {selectedRegistration.edrpou}</div>
              <div><strong>OSBB Name:</strong> {selectedRegistration.osbb_name || 'N/A'}</div>
              <div><strong>Head:</strong> {selectedRegistration.head_full_name}</div>
              <div><strong>RNOKPP:</strong> {selectedRegistration.head_rnokpp}</div>
              <div><strong>Email:</strong> {selectedRegistration.head_email}</div>
              <div><strong>Phone:</strong> {selectedRegistration.head_phone}</div>
              <div><strong>Address:</strong> {selectedRegistration.address ? JSON.stringify(selectedRegistration.address) : 'N/A'}</div>
              <div><strong>Status:</strong> {selectedRegistration.status}</div>
              <div><strong>Created:</strong> {new Date(selectedRegistration.created_at).toLocaleString()}</div>
